...
class OpStringParserTest extends GroovyTestCase {
  ...
  void testSubstrateAvailability() {
    URL url = "https://elastic-grid-substrates.s3.amazonaws.com/jboss/jboss-${version}-jdk6.zip".toURL()
    println "Testing Substrate availability on $url"
    try {
      url.newReader().readLine()
    } catch (IOException) {
  ...
  void testXmlParserOnJBossDeploymentFromFile(parser, file) {
    ...
    assertEquals "The name of the service is invalid", "JBoss", service.name
    ...
    assertEquals 'JBoss', systemComponents[0].attributes['Name']
    ...
    def StagedSoftware jboss = softwareLoads[0]
    assertTrue "Data should be removed on service destroy", jboss.removeOnDestroy()
    assertEquals "https://elastic-grid-substrates.s3.amazonaws.com/jboss/jboss-${version}-jdk6.zip", jboss.location.toString()
    assertEquals 'jboss', jboss.installRoot
    assertTrue jboss.unarchive()
